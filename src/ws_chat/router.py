...
from src.ws_chat.models import Messages
...
class ConnectionManager:
    ...
    async def broadcast(self, message: str, add_to_db=False):
        if add_to_db:
...
async def get_last_messages(session: AsyncSession = Depends(get_async_session)):
    ...
    messages_list = [msg[0].__dict__ for msg in messages]
...
async def websocket_endpoint(websocket: WebSocket, client_id: int):
    ...
    await manager.broadcast(f"Client #{client_id} added in chat")
    try:
        while True:
